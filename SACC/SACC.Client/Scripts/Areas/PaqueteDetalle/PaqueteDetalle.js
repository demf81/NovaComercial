$(function () {
    PaqueteDetalle.inicializa($("#txtApuntadorPaqueteDetalle").val());
});
...
var PaqueteDetalle = {
    ...
    inicializa:                        function (evento) {
        ...
        if (this.evento == 'PAQUETE_DETALLE_INDEX') {
            this.keyEventIndex();
            ...
            CtrlUserServicioMedicoPaqueteDetalleView.inicializa('CTRL_USER_SERVICIO_MEDICO_PAQUETE_DETALLE_VIEW');
            CtrlUserMaterialPaqueteDetalleView.inicializa('CTRL_USER_MATERIAL_PAQUETE_DETALLE_VIEW');
            CtrlUserMedicamentoPaqueteDetalleView.inicializa('CTRL_USER_MEDICAMENTO_PAQUETE_DETALLE_VIEW');
            CtrlUserEstudioPaqueteDetalleView.inicializa('CTRL_USER_ESTUDIO_PAQUETE_DETALLE_VIEW');
            CtrlUserCirugiaPaqueteDetalleView.inicializa('CTRL_USER_CIRUGIA_PAQUETE_DETALLE_VIEW');
        ...
        if (res != null)
            if (!res.data.Error)
                if (res.data.MuestraAlert)
                    setTimeout(function () { PaqueteDetalle.redirecciona('PAQUETE_DETALLE_INDEX', pPestaniaActiva); }, 2000);
                else
                    this.redirecciona('PAQUETE_DETALLE_INDEX', pPestaniaActiva);
    },
                    ...
                    if (res != null)
                        if (!res.data.Error)
                            if (res.data.MuestraAlert)
                                ...
                                PaqueteDetalle.redirecciona('PAQUETE_DETALLE_INDEX', pPestaniaActiva);
                }
                else {
                    swal.close();
                }
    ...
    keyEventIndex:                     function () {
        $('#dgDatosCtrlUserServicioMedicoPaqueteDetalleView').on('click', 'tbody tr', function (event) {
            if ($("#txtPestaniaActiva").val() == "SERVICIOS_MEDICOS") {
                var selected = $(this).hasClass("warning");

                $("#dgDatosCtrlUserServicioMedicoPaqueteDetalleView tr").removeClass("warning");
        ...
        $('#dgDatosCtrlUserMaterialPaqueteDetalleView').on('click', 'tbody tr', function (event) {
            if ($("#txtPestaniaActiva").val() == "MAERIALES") {
                var selected = $(this).hasClass("warning");

                $("#dgDatosCtrlUserMaterialPaqueteDetalleView tr").removeClass("warning");
        ...
        $('#dgDatosCtrlUserMedicamentoPaqueteDetalleView').on('click', 'tbody tr', function (event) {
            if ($("#txtPestaniaActiva").val() == "MEDICAMENTOS") {
                var selected = $(this).hasClass("warning");

                $("#dgDatosCtrlUserMedicamentoPaqueteDetalleView tr").removeClass("warning");
        ...
        $('#dgDatosCtrlUserEstudioPaqueteDetalleView').on('click', 'tbody tr', function (event) {
            if ($("#txtPestaniaActiva").val() == "ESTUDIOS") {
                var selected = $(this).hasClass("warning");

                $("#dgDatosCtrlUserEstudioPaqueteDetalleView tr").removeClass("warning");
        ...
        $('#dgDatosCtrlUserCirugiaPaqueteDetalleView').on('click', 'tbody tr', function (event) {
            if ($("#txtPestaniaActiva").val() == "CIRUGIAS") {
                var selected = $(this).hasClass("warning");

                $("#dgDatosCtrlUserCirugiaPaqueteDetalleView tr").removeClass("warning");
    ...
    redirecciona:                      function (evento, pPestaniaActiva) {
        ...
        if (evento == 'PAQUETE_DETALLE_INDEX') {
            $(location).attr('href', URL.PaqueteDetalle + '?pPaqueteId=' + $("#txtPaqueteId").val() + '&pPaqueteDescripcion=' + $("#txtPaqueteDescripcion").val() + '&pPestaniaActiva=' + pPestaniaActiva);
        };
    },
